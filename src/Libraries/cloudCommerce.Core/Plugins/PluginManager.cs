...
namespace cloudCommerce.Core.Plugins
{
    ...
    public class PluginManager
    {
		...
		public static bool IsAssumedCompatible(Version minAppVersion)
		{
			...
			if (cloudCommerceVersion.Version == minAppVersion)
			{
				return true;
			}
			...
			foreach (var version in cloudCommerceVersion.BreakingChangesHistory)
			{
        ...
        private static Assembly Probe(FileInfo plug)
        {
			if (plug.Directory == null || plug.Directory.Parent == null)
				throw new InvalidOperationException("The plugin directory for the " + plug.Name +
													" file exists in a folder outside of the allowed cloudCommerce folder hierarchy");
