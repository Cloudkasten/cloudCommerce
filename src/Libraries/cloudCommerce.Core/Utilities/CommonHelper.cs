...
namespace cloudCommerce.Utilities
{
    ...
    public static partial class CommonHelper
    {
		...
		public static string MapPath(string path, bool findAppRoot = true)
		{
			...
			else
			{
				...
				if (findAppRoot /* && !Directory.Exists(testPath)*/)
				{
					...
					if (dir != null)
					{
						baseDirectory = Path.Combine(dir.FullName, "Presentation\\cloudCommerce.Web");
		...
		public static bool IsDevEnvironment
		{
			get 
			{
				...
				// if there's a 'cloudCommerce.NET.sln' in one of the parent folders,
		...
		{
			return File.Exists(Path.Combine(dir.FullName, "cloudCommerceNET.sln"));
		}
