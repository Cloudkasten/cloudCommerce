...
namespace cloudCommerce.FacebookAuth.Core
{
	public class FacebookProviderAuthorizer : IOAuthProviderFacebookAuthorizer
	{
		...
		{
			string url = string.Format("{0}Plugins/cloudCommerce.FacebookAuth/logincallback/", _services.WebHelper.GetStoreLocation());
			return new Uri(url);
		}
