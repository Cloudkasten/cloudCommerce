...
namespace cloudCommerce.PayPal.Services
{
	public class PayPalService : IPayPalService
	{
		...
		private string ToInfoString(dynamic json)
		{
			...
			try
			{
				string[] strings = T("Plugins.cloudCommerce.PayPal.MessageStrings").Text.SplitSafe(";");
		...
		public void AddOrderNote(PayPalSettingsBase settings, Order order, string anyString, bool isIpn = false)
		{
			try
			{
				...
				string[] orderNoteStrings = T("Plugins.cloudCommerce.PayPal.OrderNoteStrings").Text.SplitSafe(";");
			...
			string providerSystemName)
		{
			...
			{
				LogError(null, T("Plugins.cloudCommerce.PayPal.FoundOrderForPayment", 0, "".NaIfEmpty()), JsonConvert.SerializeObject(json, Formatting.Indented), isWarning: true);
				return HttpStatusCode.OK;
			}
			...
			if (orders.Count != 1)
			{
				LogError(null, T("Plugins.cloudCommerce.PayPal.FoundOrderForPayment", orders.Count, paymentId), JsonConvert.SerializeObject(json, Formatting.Indented), isWarning: true);
				return HttpStatusCode.OK;
			}
			...
			{
				LogError(null, T("Plugins.cloudCommerce.PayPal.CurrencyNotEqual", currency.NaIfEmpty(), primaryCurrency), JsonConvert.SerializeObject(json, Formatting.Indented), isWarning: true);
				return HttpStatusCode.OK;
			}
