	...
	public partial class PayPalStandardProvider : PaymentPluginBase, IConfigurable
	{
		...
		public override void PostProcessPayment(PostProcessPaymentRequest postProcessPaymentRequest)
		{
            ...
            if (settings.PassProductNamesAndTotals)
			{
				...
				if (cartTotal > postProcessPaymentRequest.Order.OrderTotal)
				{
					...
					//gift card or rewared point amount applied to cart in cloudCommerce.NET - shows in Paypal as "discount"
            ...
            var returnUrl = _services.WebHelper.GetStoreLocation(store.SslEnabled) + "Plugins/cloudCommerce.PayPal/PayPalStandard/PDTHandler";
            var cancelReturnUrl = _services.WebHelper.GetStoreLocation(store.SslEnabled) + "Plugins/cloudCommerce.PayPal/PayPalStandard/CancelOrder";
            ...
            if (settings.EnableIpn)
			{
                ...
                if (String.IsNullOrWhiteSpace(settings.IpnUrl))
                    ipnUrl = _services.WebHelper.GetStoreLocation(store.SslEnabled) + "Plugins/cloudCommerce.PayPal/PayPalStandard/IPNHandler";
				else
        ...
        public void AdjustLineItemAmounts(List<PayPalLineItem> paypalItems, PostProcessPaymentRequest postProcessPaymentRequest)
        {
            try
            {
                ...
                decimal totalcloudCommerce = Math.Round(postProcessPaymentRequest.Order.OrderSubtotalExclTax, 2);
				...
				delta = Math.Round(totalcloudCommerce - totalPayPal, 2);
				//"SM: {0}, PP: {1}, delta: {2}".FormatInvariant(totalcloudCommerce, totalPayPal, delta).Dump();
				...
				//"SM: {0}, PP: {1}, delta: {2} (portion: {3}, rest: {4})".FormatInvariant(totalcloudCommerce, totalPayPal, delta, portion, rest).Dump();
			}
        ...
        public override void GetConfigurationRoute(out string actionName, out string controllerName, out RouteValueDictionary routeValues)
        {
            ...
            routeValues = new RouteValueDictionary() { { "area", "cloudCommerce.PayPal" } };
        }
