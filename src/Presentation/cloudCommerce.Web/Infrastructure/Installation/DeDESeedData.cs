    ...
    public class DeDESeedData : InvariantSeedData
    {
            ...
            settings
                ...
                .Alter<ContentSliderSettings>(x =>
                {
                    ...
                    x.Slides.Add(new ContentSliderSlideSettings
                    {
                        ...
                        Text = @"<p>cloudCommerce.NET ist die neue dynamische E-Commerce Lösung von cloudCommerce.</p>
                                 <ul>
                                     <li>Auftrags-, Kunden- und Lagerverwaltung.</li>
                                     <li>SEO-optimiert | 100% Mobile-optimiert.</li>
                                     <li>Reviews &amp; Ratings | cloudCommerce.biz Import.</li>
                                 </ul>",
                        Published = true,
                        LanguageCulture = "de-DE",
        ...
        protected override void Alter(IList<ScheduleTask> entities)
        {
            ...
            entities.WithKey(x => x.Type)
				.Alter("cloudCommerce.Services.Messages.QueuedMessagesSendTask, cloudCommerce.Services", x =>
                    {
                        x.Name = "E-Mail senden";
                    })
				.Alter("cloudCommerce.Services.Messages.QueuedMessagesClearTask, cloudCommerce.Services", x =>
					{
						x.Name = "E-Mail Queue bereinigen";
					})
				.Alter("cloudCommerce.Services.Media.TransientMediaClearTask, cloudCommerce.Services", x =>
				{
					x.Name = "Temporäre Uploads bereinigen";
				})
				.Alter("cloudCommerce.Services.Customers.DeleteGuestsTask, cloudCommerce.Services", x =>
                    {
                        x.Name = "Gastbenutzer löschen";
                    })
				.Alter("cloudCommerce.Services.Caching.ClearCacheTask, cloudCommerce.Services", x =>
                    {
				...
				.Alter("cloudCommerce.Services.Directory.UpdateExchangeRateTask, cloudCommerce.Services", x =>
                    {
                        x.Name = "Wechselkurse aktualisieren";
                    })
				.Alter("cloudCommerce.Services.Common.TempFileCleanupTask, cloudCommerce.Services", x =>
                    {
...
.Alter(19, x =>
        ...
        protected override void Alter(IList<BlogPost> entities)
        {
            ...
            .Alter("Customer Service - Client Service", x =>
            {
                ...
                x.Tags = "Shopsystem, cloudCommerce.NET, asp.net, sample tag, Service";
				x.Language = base.DbContext.Set<Language>().FirstOrDefault();
            });
        ...
        protected override void Alter(IList<NewsItem> entities)
        {
            ...
            entities.WithKey(x => x.MetaTitle)
            .Alter("cloudCommerce.NET new release!", x =>
            {
                x.Title = "cloudCommerce.NET - das clevere Shopsystem!";
                x.Short = "cloudCommerce.NET ist die neue dynamische E-Commerce Lösung von cloudCommerce. cloudCommerce.NET bietet alle Funktionen und Möglichkeiten, um schnell und einfach einen leistungsfähigen und funktional kompletten Online-Shop zu erstellen.";
                x.Full = "<p>Mit cloudCommerce.NET haben Sie alles im Griff. Verwalten Sie Ihren Lagerbestand, Ihre Aufträge und alle kundenspezifischen Funktionen, wie kundenindividuelle Rabatte, Gutscheine oder Zugriffsrechte für spezielle Kundengruppen.  </p>  <p>  Durch sprechende URL's und eine durchdachte HTML-Struktur ist cloudCommerce.NET perfekt für Suchmaschinen optimiert.  </p>  <p>  cloudCommerce.NET erkennt automatisch, ob Ihre Shopbesucher mit einem mobilen Endgerät auf Ihren Shop zugreifen und zeigt den Shop in einer Ansicht, die für geringe Auflösungen optimiert ist.  </p>  <p>  Steigern sie Ihren Umsatz und animieren Sie mehr Kunden zum Kauf mit Produkt-Rezensionen und -Bewertungen.  </p>  <p>  cloudCommerce.NET wird bereits mit Sprachpaketen für Deutsch und Englisch ausgeliefert und unterstützt die Verwaltung unendlich vieler weiterer Sprachen.  </p>  <p>  Starten Sie sofort durch!<br>  Importieren Sie Ihren cloudCommerce.biz Shop mit nur einem Klick in cloudCommerce.NET.</p>";
